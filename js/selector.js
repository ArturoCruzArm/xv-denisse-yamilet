...
const STORAGE_KEY = 'denisse_selecciones';
const KEY_FILTER  = 'denisse_filtro';
const KEY_SCROLL  = 'denisse_scroll';
const KEY_LAST    = 'denisse_ultima_foto';
...
function pintarConteos() {
    ...
    // Aviso de ampliaciones extra
    const ampExtra = Math.max(0, s.ampliacion - LIMITES.ampliacion);
...
function descargarReporte() {
    ...
    const a    = document.createElement('a');
    a.href = url;
    a.download = `seleccion-denisse-${new Date().toISOString().slice(0, 10)}.txt`;
    a.click();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (ins) ins.innerHTML = `Elige tus fotos: <strong>${LIMITES.impresion} impresiones 5x7</strong> y ` +
                             `<strong>${LIMITES.ampliacion} ampliación ${CFG.paquete.ampliacionMedida}</strong> incluidas en tu paquete.`;
